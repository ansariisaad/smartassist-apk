...
class BiometricPreference {
  ...
  static Future<Map<String, bool>> getAvailableBiometrics() async {
    ...
    try {
      ...
      // FIXED: Enhanced fallback logic for Android devices
      // Many Android devices report BiometricType.weak or BiometricType.strong instead of specific types
      bool hasSpecificTypes =
          (result['fingerprint'] ?? false) ||
          (result['face'] ?? false) ||
          (result['iris'] ?? false);
      bool hasWeakOrStrong =
          (result['weak'] ?? false) || (result['strong'] ?? false);

      print("Has specific biometric types: $hasSpecificTypes");
      print("Has weak or strong biometrics: $hasWeakOrStrong");

      if (!hasSpecificTypes) {
        // If we have weak or strong biometrics, assume it's fingerprint (most common)
        if (hasWeakOrStrong) {
          print("Device has weak/strong biometrics, mapping to fingerprint");
          result['fingerprint'] = true;
        }
        // If no specific types detected but biometrics are available, assume fingerprint
